...
while 1
    ...
    if (drive_straight == 1)
        ...
        if ((u(1) > u1_sensor_dist - u1_sensor_dist * ultrasonic_margin_err))
            % If the way ahead is clear, build command string and move bot
            % forward
            cmdstring = [strcat('d1-',num2str(speed)) newline];
            reply = tcpclient_write(cmdstring, s_cmd, s_rply);
        elseif ((u(2) > u2_sensor_dist - u2_sensor_dist * ultrasonic_margin_err))
            ...
            cmdstring = [strcat('d1-',num2str(speed)) newline];
            reply = tcpclient_write(cmdstring, s_cmd, s_rply);
        elseif ((u(5) > u5_sensor_dist - u5_sensor_dist * ultrasonic_margin_err))
             % Rotate CW ~ -90 degrees          
            cmdstring = [strcat('r1-',num2str(-90)) newline];
            reply = tcpclient_write(cmdstring, s_cmd, s_rply);
            % Drive forward 1 step
        ...
        elseif ((u(4) > u4_sensor_dist - u4_sensor_dist * ultrasonic_margin_err))
        ...
        if (drive_step_counter == 0)
            drive_step_counter = 0;
            drive_straight = 0;
        else
            drive_step_counter = drive_step_counter + 1;
        end 
