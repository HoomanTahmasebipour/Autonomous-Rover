...
function [lz_rover_centered, p, k, M, ultra] = lz_center_rover(u, s_cmd, s_rply, rover_radius, heading, p, k, M, ultra, u_loc)
    u_left = u(2);
    if (u(4) < u(5))
        u_right = u(4);
    else
    ...
    if ((u_left <= centered_dist_from_wall) || (u_right <= centered_dist_from_wall))
        lz_rover_centered = 1;
        disp('Rover centered in loading zone')
    elseif (u_left <= u_right)
        ...
        new_heading = mod(heading + 90,360);
        [u, u_real] = take_ultrasonic_measurements(s_cmd, s_rply, rover_radius, u_loc);
        [p,k,loc_y,loc_x, localized] = update_rover_location(p, M, new_heading, k);
        p = update_localization_map(u, M, p, ultra, k);
        lz_rover_centered = 0;
        disp(['Rover not centered in loading zone. Move left ' num2str(speed) ' inches'])
    else
        % turn and move right
        speed = u_right / 2;
        if (u_right > 6)
        ...
        p = update_localization_map(u, M, p, ultra, k);
        lz_rover_centered = 0;
        disp(['Rover not centered in loading zone. Move right ' num2str(speed) ' inches'])
    end
end
...
function [heading, p, k, M, ultra, loc_x, loc_y] = find_and_load_block(s_cmd, s_rply, rover_radius, u_loc, rover_dist_thresh, p, k, M, ultra, heading, ultrasonic_margin)
    ...
    if (u(2) < u(4))
        while (found_block == 0)
            ...
            if (u(1)/u(6) > detect_thresh)
                found_block = 1;
                disp("Found block!");
            %elseif (abs(heading - init_heading) >= 90)
            %    cmdstring = [strcat('d1-',num2str(1)) newline];
            %    reply = tcpclient_write(cmdstring, s_cmd, s_rply);
            %    init_heading = heading;
            %    disp("Rotated 90 degrees and havent found block, moving forward 1 inch")
            end
        end
    else
        while (found_block == 0)
    ...
    %first_unique_loc = 0;
    lz_rover_centered = 0;
    while (lz_rover_centered == 0)
        %rover_centered = center_rover(u, s_cmd, s_rply, ultrasonic_margin, rover_radius, unique_loc, first_unique_loc, heading);
        [lz_rover_centered, p, k, M, ultra] = lz_center_rover(u, s_cmd, s_rply, rover_radius, heading, p, k, M, ultra, u_loc);
        [u, u_real] = take_ultrasonic_measurements(s_cmd, s_rply, rover_radius, u_loc);
        [p,k,loc_y,loc_x, localized, max_prob] = update_rover_location(p, M, heading, k);
        p = update_localization_map(u, M, p, ultra, k);
    end
